...
def get_expected_managers_context_parts():
    return [
        "ativar cliente.txt", # From context/internal/managers/
        "ficheiro debito.txt", # From context/internal/managers/
        "Video#6 Cancelar Contrato + Ficheiro Morto.txt" # From context/internal/managers/
    ]
...
def test_managers_specific_context_loading():
    payload = {
        "message": "Test managers context loading",
        "history": [],
    ...
    if data[2]['role'] != 'user' or not data[2]['parts'][0]['text'].startswith("Here are some additional documents"):
        print("MTC Failed: 'additionalDocuments' user message not found at expected position (index 2) for managers.")
        print(f"Message at index 2: {json.dumps(data[2], indent=2)}")
        return False
    
    actual_additional_docs_content = data[2]['parts'][0]['text']
    
    prefix = "Here are some additional documents and context relevant to your current role (managers). Please use them to inform your responses:\n\n"
    if not actual_additional_docs_content.startswith(prefix):
        print(f"MTC Failed: Prefix for additional documents not found or role mismatch. Expected 'managers'.")
    ...
    managers_parts = get_expected_managers_context_parts()
    
    missing_general_parts = [part for part in general_parts if part not in actual_context_only]
    missing_managers_parts = [part for part in managers_parts if part not in actual_context_only]

    if not missing_general_parts and not missing_managers_parts:
        print("MTC Success: Managers context (general + managers-specific) loaded correctly (all key parts found).")
        return True
    else:
        print("MTC Failed: Managers context not loaded correctly.")
        if missing_general_parts:
            for part in missing_general_parts:
                 print(f"- Missing general part: '{part}'")
        if missing_managers_parts:
            for part in missing_managers_parts:
                 print(f"- Missing managers-specific part: '{part}'")
        print(f"Actual combined context (first 1000 chars):\n---\n{actual_context_only[:1000]}\n---")
        return False

if __name__ == "__main__":
    if not test_managers_specific_context_loading():
        print("test_managers_specific_context_loading returned False.")
        exit(1)
    else:
        print("test_managers_specific_context_loading returned True.")
